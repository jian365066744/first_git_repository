 ...
 *******************************************************************************/
package org.modules.nosql.redis;
...
import org.modules.nosql.redis.pool.JedisPool;
...
public class JedisTemplate {
	...
	public Boolean delete(final String key) {
		return execute(new JedisAction<Boolean>() {
			...
			@Override
			public Boolean action(Jedis jedis) {
				return jedis.del(SerializationUtils.serialize(key)) == 1 ? true : false;
			}
		});
	}
	...
	public void setex(final String key, final Object value, final int seconds) {
		execute(new JedisActionNoResult() {
			...
			@Override
			public void action(Jedis jedis) {
				jedis.setex(SerializationUtils.serialize(key), seconds, SerializationUtils.serialize((Serializable)value));
			}
		});
	}
	...
	public void set(final String key, final Object value) {
		execute(new JedisActionNoResult() {
			@Override
			public void action(Jedis jedis) {
				jedis.set(SerializationUtils.serialize(key), SerializationUtils.serialize((Serializable)value));
			}
		});
	}
	...
	public Object getAsObject(final String key) {
		...
		byte[] value = execute(new JedisAction<byte[]>() {
			@Override
			public byte[] action(Jedis jedis) {
				return jedis.get(SerializationUtils.serialize(key));
			}
		});
		if(null != value){
			returnValue = SerializationUtils.deserialize(value);
		}
		return returnValue;
	}
}
